(function () {
  ...
  function initializeSourceModes() {
    ...
    els.driveConnectionDetail.textContent = configured
      ? 'Acceso de solo lectura. El token se conserva únicamente durante la sesión.'
      : 'Complete googleOAuthClientId en js/config.js para habilitar esta modalidad.';
    setSourceMode('LOCAL', { preserveFiles: true });
  }
  ...
  function setSourceMode(mode, options = {}) {
    ...
    els.sourceModeStatus.textContent = drive ? 'Modo Drive activo' : 'Modo local activo';
    els.modeHeaderChip.innerHTML = drive ? '<i></i>Procesamiento desde Drive' : '<i></i>Procesamiento local';
    els.privacyText.textContent = drive
      ? 'Los archivos se descargan directamente desde Google Drive a la memoria de esta sesión del navegador; no se almacenan en servidores de la aplicación.'
  ...
  async function connectDrive() {
    clearMessage();
    try {
      if (!D || !D.isConfigured || !D.isConfigured()) throw new Error('Google Drive no está configurado. Completa js/config.js.');
      setDriveStatus('connecting', 'Conectando con Google Drive…', 'Autoriza acceso de solo lectura en la ventana de Google.');
      await D.connect();
      setDriveStatus('connected', 'Google Drive conectado', 'Sesión autorizada temporalmente con acceso de solo lectura.');
      els.driveConnectButton.hidden = true;
      els.driveDisconnectButton.hidden = false;
      els.driveScanButton.disabled = false;
      showMessage('success', 'Conexión con Google Drive establecida. Indica la carpeta y busca los reportes.');
    } catch (error) {
      console.error(error);
      setDriveStatus('error', 'No se pudo conectar Drive', String(error.message || error));
      showMessage('error', String(error.message || 'No se pudo conectar con Google Drive.'));
    }
  }
  ...
  async function disconnectDrive() {
    ...
    renderDriveFiles();
    setDriveStatus('idle', 'Drive no conectado', 'Conecta nuevamente para consultar la carpeta.');
  ...
  async function scanDriveFolder() {
    ...
    try {
      ...
      if (!state.driveFolderId) throw new Error('Ingresa el enlace o ID de la carpeta de Google Drive.');
      els.driveScanButton.disabled = true;
